(function($, Drupal) {
  ...
  Drupal.behaviors.clarin_theme_adminbar_helper = {
    attach: function(context, settings) {
      ...
      function repositionContent(mainToolbar, header, body, toolbarElem, timer) {
        ...
        if (toolbarElem.id === "toolbar-bar" ) {
          ...
          if (toolbarElem.clientHeight === 0) {
            ...
            toolbarTotalHeight = Math.floor(toolbarElem.offsetTop + toolbarElem.clientHeight);
          } else {
        ...
        const headerObservedStickyTop = Math.floor(parseFloat(header.css("top")));
        let topOffset = Math.abs(headerObservedStickyTop - prevEffectiveStickyTop);
          ...
          $("#toolbar-bar, .toolbar-tray", body).each((index, adminBarElement) => {
            ...
            const trayResizeObserver = new ResizeObserver(entries => {
              for (let entry of entries) {
                repositionContent(mainToolbar, header, body, entry.target, timer);
              }
            });
